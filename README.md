...
a map and a busiest-areas table.

**The charts and the map are filters.** Click a month bar to zoom the period to that month;
...
A choropleth of alert counts: oblasts for the whole country, and the raions of
one oblast once you pick it. Hovering names an area and gives its figures,
clicking filters to it, and clicking off the shapes clears the area filter, the
same as the other charts. It is drawn from the **same `ranking()` call as the
table underneath**, so the two cannot drift apart.

Colour steps are **quantiles**, not a linear scale. The busiest oblast has had
more than twenty times the quietest, and a linear ramp would paint two areas
dark and everything else the palest step. The legend states the real range of
each step.

Areas with no figures stay grey and are not clickable — Crimea and Sevastopol,
which the feed does not cover, and any raion that has never had a siren
declared for it specifically. Picking one would set a value the dropdown does
not have.

### Where the boundaries come from

[`slawomirmatuszak/ukrainian_geodata`](https://github.com/slawomirmatuszak/ukrainian_geodata)
(**CC BY 4.0** — credited in the page footer, as the licence requires).

It has to be a post-2020 source. The 2020 reform cut 490 raions to 136, and the
boundary sets that are easiest to reach still ship the old ones — geoBoundaries'
ADM2 for Ukraine is 495 units dated 2006, which matches almost nothing in this
dataset.

The feed names areas in Latin script (`Bakhmutskyi raion`) and every boundary
file names them in Ukrainian (`Бахмутський район`). Rather than hand-maintain a
143-row lookup table, `geo.py` transliterates with the **Ukrainian National
standard** (KMU resolution 55, 2010) — the one the feed itself appears to use.
That matches 24 of 25 oblasts and 116 of 118 raions outright, and `область` and
`район` fall out as `oblast` and `raion` without special-casing. The exceptions
are named explicitly:

| | |
|---|---|
| `Novomoskovskyi` → `Samarivskyi` | renamed 2024, after the boundary file |
| `Novohrad-Volynskyi` → `Zviahelskyi` | renamed 2023, likewise |
| Kyiv City | absent as a region; it is the hole in Kyivska oblast |

Kyiv is taken from that hole rather than another source, which keeps the whole
map on one licence. It is identified by extent and area — 846 km² against the
city's 839 — not by assuming a ring order.

`build.py` fails if any area in the data has no shape to draw, so an upstream
rename surfaces as a failed build rather than a quiet hole in the map.

Geometry is projected, simplified and quantised at build time, so the browser
only draws path strings: 376 kB of JSON, which the host compresses on the way
out. Simplification runs at well under a pixel at the size drawn, so the
slivers it opens along shared borders are invisible and covered by the stroke.
...
data/                cached CSV and boundary files (gitignored)
```
